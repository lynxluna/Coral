...
import com.bhinneka.coral.core.EventStore;
...
public class InMemoryEventStore<I, S> implements EventStore<I, S> {
  ...
  public void load(final I id, final AsyncHandler<List<EventInfo<I, S>>> handler) {
    if (storage.size() == 0) {
      ...
      final List<EventInfo<I, S>> eventInfos = CollectionUtilities.filter(storage, new CollectionUtilities.Predicate<EventInfo<I, S>>() {
        @Override
  ...
  public void load(final I id,
    ...
    this.load(id, new AsyncHandler<List<EventInfo<I, S>>>() {
      ...
      public void onSuccess(@Nonnull List<EventInfo<I, S>> result) {
        final List<EventInfo<I, S>> infos = CollectionUtilities.filter(result, new CollectionUtilities.Predicate<EventInfo<I, S>>() {
          @Override
          public boolean apply(EventInfo<I, S> type) {
